    ...
    page.innerHTML = `
      <div style="max-width:560px;width:100%;">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:28px">
          <button onclick="history.back();setTimeout(function(){var p=document.getElementById('wc-qb-login-page');if(p)p.remove();var bd=document.querySelector('.fixed.inset-0.z-10');if(bd)bd.style.display='none';},150);(function tryClose(n){setTimeout(function(){var xBtn=Array.from(document.querySelectorAll('.md\\:hidden button')).find(function(b){return Array.from(b.querySelectorAll('path')).some(function(p){return(p.getAttribute('d')||'').includes('18 6 6 18');});});if(xBtn){var r=xBtn.getBoundingClientRect();xBtn.dispatchEvent(new PointerEvent('click',{bubbles:true,cancelable:true,composed:true,clientX:r.x+r.width/2,clientY:r.y+r.height/2,view:window}));}else if(n>0){tryClose(n-1);}},300);})(5);" style="display:flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;border:1px solid hsl(var(--border));background:hsl(var(--card));cursor:pointer;flex-shrink:0;" title="Back">
