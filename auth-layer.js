    ...
    page.innerHTML = `
      <div style="max-width:560px;width:100%;">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:28px">
          <button onclick="history.go(-2);setTimeout(function(){var p=document.getElementById('wc-qb-login-page');if(p)p.remove();var bd=document.querySelector('.fixed.inset-0.z-10');if(bd)bd.style.display='none';},200);setTimeout(function(){var xBtn=Array.from(document.querySelectorAll('.md\\:hidden button')).find(function(b){return Array.from(b.querySelectorAll('path')).some(function(p){var d=p.getAttribute('d')||'';return d.includes('18 6 6 18')||d.includes('6 6 12 12');});});if(xBtn){var r=xBtn.getBoundingClientRect();var ev=new PointerEvent('click',{bubbles:true,cancelable:true,composed:true,clientX:r.x+r.width/2,clientY:r.y+r.height/2,view:window});xBtn.dispatchEvent(ev);}},500);" style="display:flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;border:1px solid hsl(var(--border));background:hsl(var(--card));cursor:pointer;flex-shrink:0;" title="Back">
